...
public class LoginController {
	...
	/**
	 * @Title: logIn @Description: TODO 验证用户 @param: request @param:
	 *         response @return: Integer @throws
	 */
	@RequestMapping("/login")
	@ResponseBody
	public Integer logIn(HttpServletRequest request, HttpServletResponse response, @FormModel("user") User user) {
		try {
			User byUser = userServer.getByUser(user);
			if (byUser == null) {
				return -1;
			} else {
	...
	/**
	 * @Title: loginOut @Description: TODO 退出系统 @return: String @throws
	 */
	...
	/**
	 * @Title: validateCaptcha @Description: TODO 验证码验证 @param: request @param:
	 *         response @param: authCode @return: Integer @throws
	 */
